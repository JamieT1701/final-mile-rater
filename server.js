...
async function getZoneByZipCode(zipCode) {
  try {
    const result = await pool.query('SELECT zone FROM zip_to_zone WHERE zip_code = $1', [zipCode]);
...
async function calculateRate(zipCode, weight) {
  ...
  try {
    const result = await pool.query('SELECT rate FROM pricing_matrix WHERE zone = $1 AND weight_min <= $2 AND weight_max >= $2', [zone, weight]);
